...
class HomeCubit extends Cubit<HomeState> {
  ...
  Future<void> loadForecast({bool silent = false}) async {
    ...
    try {
      ...
      await _loadForecastForCity(selectedCity);
    } catch (_) {
      emit(
        state.copyWith(
          loading: false,
  ...
  Future<void> searchCity(String query) async {
    ...
    if (trimmedQuery.length < 2) {
      emit(state.copyWith(cityResults: [], searchingCity: false));
      return;
    }

    emit(state.copyWith(searchingCity: true, clearError: true));
  ...
  Future<void> changeCity(AppCity city) async {
    emit(state.copyWith(loading: true, cityResults: [], clearError: true));

    try {
      await cityPreferencesUtils.saveSelectedCity(city);
      await _loadForecastForCity(city);
    } catch (_) {
      emit(
        state.copyWith(loading: false, error: 'Impossibile cambiare città.'),
      );
    }
  }
  ...
  Future<void> scheduleReminder({
    ...
    required String Function(String city, String bestTimeRange, String peakHour)
    notificationBodyBuilder,
    required String notificationEndTitle,
    required String Function(String city, String bestTimeRange)
  ...
  Future<void> cancelTomorrowReminder() async {
    ...
    if (state.reminderActiveTomorrow) {
  ...
  Future<void> _scheduleReminderForDate({
    ...
    final advice = solarAdviceUtils.calculate(forecast, targetDate: targetDate);
    ...
    emit(state.copyWith(reminderDateTime: reminderStateDateTime));
  }

  Future<void> _loadForecastForCity(AppCity city) async {
  ...
  DateTime _getReminderDateTimeFromRange({
    ...
    var scheduledTime = startTime.subtract(const Duration(minutes: 10));
  ...
  DateTime _getRangeEndTime({
    ...
    return DateTime(targetDate.year, targetDate.month, targetDate.day, endHour);
  }
...
enum _ReminderTiming { beforeRange, eveningBefore }
